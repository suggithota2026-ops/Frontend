          ...
          {selectedNotification && (
            <div className="relative">
              ...
              <div className="px-6 py-5 bg-muted/30 backdrop-blur-sm border-t space-y-3">
                ...
                {/* WhatsApp Redirect Button - For contact messages */}
                {selectedNotification.metadata?.contactNumber && (
                  <div className="flex justify-center">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => {
                        const number = selectedNotification.metadata.contactNumber.replace(/\D/g, '');
                        // Ensure matching the format for wa.me (91 for India if not present, but usually backend stores 10 digits)
                        const formattedNumber = number.length === 10 ? `91${number}` : number;
                        window.open(`https://wa.me/${formattedNumber}?text=Hello, this is PRK Smiles Admin. We received your inquiry.`, '_blank');
                      }}
                      className="h-9 px-4 border-[#25D366] text-[#25D366] hover:bg-[#25D366] hover:text-white shadow-sm transition-all duration-200"
                    >
                      <WhatsAppIcon className="w-4 h-4 mr-2" />
                      <span className="font-bold uppercase tracking-tight text-[10px]">Respond on WhatsApp</span>
                    </Button>
                  </div>
                )}
